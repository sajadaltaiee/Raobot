...
local function check_member_superrem(cb_extra, success, result)
  ...
  for k,v in pairs(result) do
    ...
    if member_id ~= our_id then
      ...
      save_data(_config.moderation.data, data)
	  local text = 'ليش عطلوتني بالمجموعة😢\n\n والله فقير خوش بوت😢🙏🏻'
      return reply_msg(msg.id, text, ok_cb, false)
    end
  end
end
...
local function lock_group_arabic(msg, data, target)
  ...
  if group_arabic_lock == 'yes' then
    return "اللغة العربية مقفولة لتلح🔒✅💯\n\n بواسطة : @"..msg.from.username
  else
...
local function unlock_group_arabic(msg, data, target)
  ...
  if group_arabic_lock == 'no' then
    return "اللغة العربية مفتوحه لتلح🔓✋🏻💯\n\n بواسطة : @"..msg.from.username
  else
...
local function lock_group_rtl(msg, data, target)
  ...
  if group_rtl_lock == 'yes' then
    return "اضافة الجماعه مقفوله لتلح🔒✋🏻💯\n\n بواسطة : @"..msg.from.username
  else
    data[tostring(target)]['settings']['lock_rtl'] = 'yes'
    save_data(_config.moderation.data, data)
    return "تم قفل اضافة الجماعه🔒✅💯\n\n بواسطة : @"..msg.from.username
  end
end
...
local function unlock_group_rtl(msg, data, target)
  ...
  if group_rtl_lock == 'no' then
    return "اضافة الجماعه مفتوحه لتلح🔓✋🏻💯\n\n بواسطة : @"..msg.from.username
  else
    data[tostring(target)]['settings']['lock_rtl'] = 'no'
    save_data(_config.moderation.data, data)
    return "تم فتح اضافة الجماعه🔓✅💯\n\n بواسطة : @"..msg.from.username
  end
end
...
function get_message_callback(extra, success, result)
	...
	elseif get_cmd == "تنزيل مشرف" then
		local user_id = result.from.peer_id
		local channel_id = "channel#id"..result.to.peer_id
...
local function run(msg, matches)
	...
	elseif msg.to.type == 'channel'then
		if matches[1] == 'خارقة' then
			...
			return "المجموعة خارقة وداعت حبنه😑💔"
		end
	end
	...
	local name_log = print_name:gsub("_", " ")
	local data = load_data(_config.moderation.data)
		if matches[1] == 'ويق' and not matches[2] then
			if not is_admin1(msg) and not is_support(support_id) then
				return
			end
			if is_super_group(msg) then
				return reply_msg(msg.id, 'البوت مفعل بكروبك😒\n\n كافي ملحه ملح💔😒', ok_cb, false)
			end
		...
		if matches[1] == 'بيق' and is_admin1(msg) and not matches[2] then
    ...
    return "تم ارسال الرابط الى الخاص 😚👍"
      ...
      if matches[1] == 'صنع مجموعه' and matches[2] then
		...
		if matches[1] == 'تغير الرابط' and is_owner(msg) then
			data[tostring(msg.to.id)]['settings']['set_link'] = 'waiting'
			save_data(_config.moderation.data, data)
			return 'ارسلي رابط  الكروب حتى اجدده❤️👍\n\n واذا متعرف روح ع اضافة الاعضاء واضافة باستخدام رابط الدعوه\n\n انسخ الرابط ودزه🐸👌🏼'
		end
		...
		if matches[1] == 'الرابط' then
			...
			local group_link = data[tostring(msg.to.id)]['settings']['set_link']
			if not group_link then
				return "اول شي لازم دز امر | تغير الرابط |\n\n وراها اكدر اطلعلك الرابط💞😁"
			end
			savelog(msg.to.id, name_log.." ["..msg.from.id.."] requested group link ["..group_link.."]")
			return "رابط ارقى كروب😍❤️👇🏻:\n"..group_link
		end
		...
		--[[if matches[1] == 'kick' and is_momod(msg) then
			...
			elseif matches[1] == 'وضع مشرف' and string.match(matches[2], '^%d+$') then
			--[[]	local receiver = get_receiver(msg)
				local user_id = "user#id"..matches[2]
				local get_cmd = 'setadmin'
				user_info(user_id, cb_user_info, {receiver = receiver, get_cmd = get_cmd})]]
				local	get_cmd = 'وضع مشرف'
				local	msg = msg
				local user_id = matches[2]
				channel_get_users (receiver, in_channel_cb, {get_cmd=get_cmd, receiver=receiver, msg=msg, user_id=user_id})
			elseif matches[1] == 'وضع مشرف' and not string.match(matches[2], '^%d+$') then
				--[[local cbres_extra = {
					channel = get_receiver(msg),
					get_cmd = 'setadmin'
				}
		...
		if matches[1] == 'تنزيل مشرف' then
			...
			elseif matches[1] == 'تنزيل مشرف' and string.match(matches[2], '^%d+$') then
				local receiver = get_receiver(msg)
				local user_id = "user#id"..matches[2]
				local get_cmd = 'تنزيل مشرف'
				user_info(user_id, cb_user_info, {receiver = receiver, get_cmd = get_cmd})
			elseif matches[1] == 'تنزيل مشرف' and not string.match(matches[2], '^%d+$') then
				local cbres_extra = {
					channel = get_receiver(msg),
					get_cmd = 'تنزيل مشرف'
				}
		...
		if matches[1] == 'وضع مشرف' and is_owner(msg) then
			if type(msg.reply_id) ~= "nil" then
				local cbreply_extra = {
					get_cmd = 'setowner',
					msg = msg
		...
		if matches[1] == 'وضع مدير' then
		  if not is_momod(msg) then
			...
			elseif matches[1] == 'وضع مدير' and string.match(matches[2], '^%d+$') then
				local receiver = get_receiver(msg)
				local user_id = "user#id"..matches[2]
				local get_cmd = 'promote'
				savelog(msg.to.id, name_log.." ["..msg.from.id.."] promoted user#id"..matches[2])
				user_info(user_id, cb_user_info, {receiver = receiver, get_cmd = get_cmd})
			elseif matches[1] == 'وضع مدير' and not string.match(matches[2], '^%d+$') then
				local cbres_extra = {
					channel = get_receiver(msg),
					get_cmd = 'promote',
				}
		...
		if matches[1] == 'تنزيل مدير' then
			...
			elseif matches[1] == 'تنزيل مدير' and string.match(matches[2], '^%d+$') then
				local receiver = get_receiver(msg)
				local user_id = "user#id"..matches[2]
		...
		if matches[1] == 'قفل' and is_momod(msg) then
			...
			if matches[2]:lower() == 'اضافة الاعضاء' then
		...
		if matches[1] == 'قفل' and is_owner(msg) then
			...
			local msg_type = 'Gifs'
				if not is_muted(chat_id, msg_type..': yes') then
					savelog(msg.to.id, name_log.." ["..msg.from.id.."] set SuperGroup to: mute "..msg_type)
					mute(chat_id, msg_type)
					return "تم قفل الصور المتحركه🔒✅💯\n\n بواسطة : @"..msg.from.username
				else
					return "الصور المحتركه مقفولة لتلح 🔒✋🏻💯\n\n بواسطة : @"..msg.from.username
				end
			end
			...
			local msg_type = 'Documents'
				if not is_muted(chat_id, msg_type..': yes') then
					savelog(msg.to.id, name_log.." ["..msg.from.id.."] set SuperGroup to: mute "..msg_type)
					mute(chat_id, msg_type)
					return "تم قفل ارسال ملفات🔒✅💯\n\n بواسطة : @"..msg.from.username
				else
					return "منع ارسال الملفات مقفول لتلح🔒✋🏻💯\n\n بواسطة : @"..msg.from.username
				end
			end
			...
			local msg_type = 'All'
				if not is_muted(chat_id, msg_type..': yes') then
					savelog(msg.to.id, name_log.." ["..msg.from.id.."] set SuperGroup to: mute "..msg_type)
					mute(chat_id, msg_type)
					return "تم قفل الجميع🔇🔕🔒\n\n بواسطة : @"..msg.from.username
				else
					return "الجميع مقفول لتلح 🔕🔇🔒💯\n\n بواسطة : @"..msg.from.username
				end
			end
		end
		if matches[1] == 'فتح' and is_momod(msg) then
			...
			local msg_type = 'Gifs'
				if is_muted(chat_id, msg_type..': yes') then
					...
					return "الصور المتحركه مفتوحه لتلح🔓✋🏻💯\n\n بواسطة : @"..msg.from.username
				end
			end
			if matches[2] == 'الملفات' then
			local msg_type = 'Documents'
				if is_muted(chat_id, msg_type..': yes') then
					savelog(msg.to.id, name_log.." ["..msg.from.id.."] set SuperGroup to: unmute "..msg_type)
					unmute(chat_id, msg_type)
					return "تم فتح ارسال الملفات🔓✅💯\n\n بواسطة : @"..msg.from.username
				else
					return "ارسال الملفات مفتوحه لتلح🔓✋🏻💯\n\n بواسطة : @"..msg.from.username
				end
			end
			if matches[2] == 'الدردشة' then
			local msg_type = 'Text'
			...
			local msg_type = 'All'
				if is_muted(chat_id, msg_type..': yes') then
					...
					return "الكل مفتوح لتلح 🔓✅✅\n\n بواسطة : @"..msg.from.username
				end
			end
		end


		if matches[1] == "نلصم" and is_momod(msg) then
			local chat_id = msg.to.id
			local hash = "mute_user"..chat_id
			local user_id = ""
			if type(msg.reply_id) ~= "nil" then
			...
			elseif matches[1] == "نلصم" and string.match(matches[2], '^%d+$') then
				local user_id = matches[2]
				if is_muted_user(chat_id, user_id) then
					unmute_user(chat_id, user_id)
			...
			elseif matches[1] == "نلصم" and not string.match(matches[2], '^%d+$') then
				local receiver = get_receiver(msg)
				local get_cmd = "mute_user"
				local username = matches[2]
				local username = string.gsub(matches[2], '@', '')
		...
		if matches[1] == 'اعدادات الكروب' and is_momod(msg) then
			local target = msg.to.id
