...
local support_id = msg.from.id
 if matches[1]:lower() == 'id' and msg.to.type == "chat" or msg.to.type == "user" then
  ...
  if matches[1]:lower() == "المحضورين" then -- Ban list !
    local chat_id = msg.to.id
  ...
  if matches[1]:lower() == 'رفع الحضر' then -- /unban
    if type(msg.reply_id)~="nil" and is_momod(msg) then
...
if matches[1]:lower() == 'دي' then
  ...
  if matches[1]:lower() == 'رفع العام' then -- Global unban
    local user_id = matches[2]
    local chat_id = msg.to.id
  ...
  if matches[1]:lower() == "المحضورين عام" then -- Global ban list
    return banall_list()
  end
end
